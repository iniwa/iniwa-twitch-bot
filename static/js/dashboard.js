(function() {
                ...
                if (data.viewers.length === 0) {
                    ...
                    tb.innerHTML = data.viewers.map(function(v) {
                        ...
                        return '<tr>' +
                            ...
                            '<td>' + v.total + '\u56DE</td>' +
                            '<td><button class="btn-memo" title="' + memoEsc + '" data-action="open-memo" data-uid="' + escHtml(v.uid) + '" data-name="' + escHtml(v.name) + '" data-memo="' + memoEsc + '">\uD83D\uDCDD</button></td>' +
                            '<td><button class="btn-so" data-action="shoutout" data-login="' + escHtml(v.login) + '" data-name="' + escHtml(v.name) + '">SO</button></td>' +
                            '</tr>';
                    }).join('');
                }
            }
        ...
        return fetch('/api/history').then(function(r) { return r.json(); }).then(function(data) {
            ...
            tbody.innerHTML = data.map(function(v) {
                ...
                return '<tr>' +
                    ...
                    '<button class="btn-memo" data-action="open-memo" data-uid="' + escHtml(v.uid) + '" data-name="' + escHtml(v.name) + '" data-memo="' + memoEsc + '">\uD83D\uDCDD</button></td>' +
    ...
    document.addEventListener('click', function(e) {
        var memoBtn = e.target.closest('[data-action="open-memo"]');
        if (memoBtn) {
            openMemoModal(memoBtn.dataset.uid, memoBtn.dataset.name, memoBtn.dataset.memo || '');
            return;
        }
        var soBtn = e.target.closest('[data-action="shoutout"]');
        if (soBtn) {
            shoutout(soBtn.dataset.login, soBtn.dataset.name);
            return;
        }
        var presetBtn = e.target.closest('[data-action="edit-preset"]');
        if (presetBtn) {
            var d = presetBtn.dataset;
            openEditPreset(parseInt(d.index), d.name, d.game, d.title, d.tags, d.tweetTags || '');
            return;
        }
    });

    // ============================================================
    // Expose initLayoutM globally (called by common.js setLayout)
    // ============================================================
    window.initLayoutM = initLayoutM;
